...
func TarGzs(files []File, target string) {
	// Create output file
...
func toTarGzs(files []File, buf1 io.Writer) {
...
func ExtractTarGzs(gzipStream io.Reader, target string) {
	...
	for true {
		...
		case tar.TypeReg:
			outFile, err := os.Create(fmt.Sprintf("%s%s", target, header.Name))
			if err != nil {
...
func addToTarGz(tw *tar.Writer, filename string) error {
	...
	// fmt.Println(strings.Replace(filename, "./upload/", "", 1))
	// header.Name = strings.Replace(filename, "./upload/", "", 1)
